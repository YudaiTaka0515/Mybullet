...
planeOrn = [0, 0, 0, 1]
# p.getQuaternionFromEuler([0.3,0,0])
...
path = r"D:\Project\bullet3\examples\pybullet\gym\pybullet_data\cloth_z_up.obj"
clothId = p.loadSoftBody(path, basePosition=[0, 0, 5], scale=0.5, mass=1., useNeoHookean=0,
...
p.createSoftBodyAnchor(clothId, 4, -1, -1)
p.createSoftBodyAnchor(clothId, 21, -1, -1)
p.createSoftBodyAnchor(clothId, 16, -1, -1)
p.createSoftBodyAnchor(clothId, 17, -1, -1)
p.createSoftBodyAnchor(clothId, 18, -1, -1)
...
# p.createSoftBodyAnchor(clothId, 18, -1, -1)
# p.createSoftBodyAnchor(clothId, 14, -1, -1)
# p.createSoftBodyAnchor(clothId, 15, -1, -1)
# p.createSoftBodyAnchor(clothId, 13, -1, -1)
# p.createSoftBodyAnchor(clothId, 17, -1, -1)
...
while p.isConnected():
    ...
    if debug:
        data = p.getMeshData(clothId, -1, flags=p.MESH_DATA_SIMULATION_MESH)
        # data = p.getMeshData(clothId, -1)
        for i in range(data[0]):
            pos = data[1][i]
            # uid = p.addUserDebugText(str(i), pos, textColorRGB=[1, 1, 1], replaceItemUniqueId=text_uid[i])
            # uid = p.addUserDebugText(str(i), pos, replaceItemUniqueId=text_uid[i])
